...
export class BluetoothConnectPage{
  ...
  //ENVIAR DATOS 
  async successSendData(header, msg: string) {
    const alert = await this.alertController.create({
      header: header,
      message: msg,
      buttons: ['OK']
  ...
  async failSendData(header, msg: string) {
    const alert = await this.alertController.create({
      header: header,
      message: msg,
      buttons: ['OK']
  ...
  sendData(dataToSend: string) {
    ...
    this.bluetoothSerial.write(this.dataSend).then(success => {
      this.successSendData('Comando enviado!', this.dataSend);
    }, error => {
      this.failSendData('Error al enviar comando', 'intentelo nuevamente');
    });
  }
  ...
  /*readData(){
    this.bluetoothSerial.read().then(success => {
      this.successSendData('Lectura de datos', success);
    }, failure => {
      this.failSendData('Error al leer datos', failure);
    });
  }*/
